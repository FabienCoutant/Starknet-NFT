...
# uint256_lt, fonction pour comparer notre max_supply avec notre id courant
from starkware.cairo.common.uint256 import Uint256, uint256_lt
from starkware.cairo.common.bool import TRUE
from starkware.starknet.common.syscalls import get_caller_address

from openzeppelin.introspection.ERC165 import ERC165_supports_interface
# uint256_checked_add, fonction pour ajouter des Uint256
...
# Storage
#
# on définit une variable pour notre ID Courant
@storage_var
func current_id_storage() -> (id : Uint256):
end

# on définit une variable pour la max_supply de notre projet (max_supply = maximum de NFT qu'on peut créer)
@storage_var
func max_supply_storage() -> (maxSupply : Uint256):
end
...
func constructor{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    max_supply_storage.write(max_supply)
    return ()
end
...
@external
func mint_symbiote{pedersen_ptr : HashBuiltin*, syscall_ptr : felt*, range_check_ptr}():
    alloc_locals
    # on récupère l'adresse du wallet qui sollicite le mint
    let (caller) = get_caller_address()
    # On récupère notre id courant
    let (id: Uint256) = current_id_storage.read()
    # on récupère la max supply
    let (maxSupply: Uint256) = max_supply_storage.read()
    # On vérifie si la max supply est dépassée ou non
    let (max_supply_pas_depassee) = uint256_lt(id , maxSupply)
    # on lève une erreur si la max supply est dépassée
    with_attr error_message("ERC721: All symbiote minted"):
        assert max_supply_pas_depassee = TRUE
    end
    # on mint notre symbiote
    ERC721._mint(caller, id)
    # on met à jour le nouvel id
    let (new_id : Uint256) = uint256_checked_add(id, Uint256(1, 0))
    current_id_storage.write(new_id)
    return ()
end